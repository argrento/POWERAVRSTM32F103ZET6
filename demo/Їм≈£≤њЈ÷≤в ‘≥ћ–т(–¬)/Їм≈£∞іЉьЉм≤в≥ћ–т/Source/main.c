...
int main(void)
{
  ...
  {	
	...
	{
		GPIO_ResetBits(GPIOF, GPIO_Pin_6);         //D1亮
		Delay(0xAFFFF);
	}

	if(GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8)==0)   //如果PA8 = 0
	{
		GPIO_ResetBits(GPIOF, GPIO_Pin_8);         //D2亮
		Delay(0xAFFFF);
	}

	if(GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13)==0)   //如果PC13 = 0
	{
		GPIO_ResetBits(GPIOF, GPIO_Pin_7);         //D3亮
		Delay(0xAFFFF);
	}

	if(GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_3)==0)   //如果PD3 = 0
	{
		GPIO_SetBits(GPIOF, GPIO_Pin_6);
		GPIO_SetBits(GPIOF, GPIO_Pin_7);
		GPIO_SetBits(GPIOF, GPIO_Pin_8);
		GPIO_SetBits(GPIOF, GPIO_Pin_9);			  // D1.D2.D3全灭
		Delay(0xAFFFF);
	}
  }
}
